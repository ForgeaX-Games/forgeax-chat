...
const labels = {
  en: { started: 'Compacting context', completed: 'Context compacted', failed: 'Context compaction failed' },
  zh: { started: '正在整理上下文', completed: '上下文已整理', failed: '上下文整理失败' },
};
...
export function formatCompactionStatus(event: StoredEvent): SystemMessage | null {
  ...
  if (p.phase !== 'started' && p.phase !== 'completed' && p.phase !== 'failed') return null;
